...
class MixDataset(Dataset):
    def __init__(self, X, sent_vecs, sender, vocabulary, vocabulary_vecs, tfidf_bow=None):
        self.X = X
        self.vecs = sent_vecs
        self.sender = sender
        self.tfidf = tfidf_bow

        self.vocabulary_vecs = vocabulary_vecs
        self.idx2token = vocabulary
    ...
    def __getitem__(self, i):
        ...
        X = torch.FloatTensor(self.X[i])
        vecs = torch.FloatTensor(self.vecs[i])
        sender = torch.FloatTensor(self.sender[i])
        if len(self.tfidf):
            tfidf = torch.FloatTensor(self.tfidf[i])
        else:
            tfidf = X
        return {'X': X, 'vecs': vecs, 'sender': sender, 'tfidf': tfidf}
